/* tree-5_1.c --
   ...
   $Id: tree-5_1.c,v 1.1.1.1 2003/12/11 08:51:02 marco Exp $
*/
...
test (void)
{
  ...
  */
  ...
  assert(ucl_tree_get_next(node) == n[3]);
  assert(ucl_tree_get_son(node)  == n[6]);

  assert(ucl_tree_get_dad(n[3])  == n[0]);
  assert(ucl_tree_get_next(n[3]) == n[4]);
  assert(ucl_tree_get_prev(n[3]) == n[1]);
