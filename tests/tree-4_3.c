/* tree-4_3.c --
   ...
   $Id: tree-4_3.c,v 1.1.1.1 2003/12/11 08:46:49 marco Exp $
*/
...
test (void)
{
  ...
  */
  ...
  assert(ucl_tree_get_dad(node)  == n[14]);
  assert(ucl_tree_get_prev(node) == NULL);
  assert(ucl_tree_get_next(node) == n[16]);
  assert(ucl_tree_get_son(node)  == NULL);

  assert(ucl_tree_get_prev(n[16]) == node);
