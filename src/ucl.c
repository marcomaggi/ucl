	       ...
	       ucl_value_comparison_t cmp)
{
  ...
  if (total_elems > MAX_THRESH) {
    ...
    while (STACK_NOT_EMPTY) {
	 ...
	 and  HI  so  the  three  values are  sorted.  This  lowers  the
	 probability of  picking a pathological pivot value  and skips a
	 comparison  for both the  LEFT_PTR and  RIGHT_PTR in  the while
	 loops. */
    ...
    jump_over:;
	 ...
	 that this algorithm runs much faster than others. */
      do {
	while (cmp.func(cmp.data, left_ptr, mid) < 0) {
	  left_ptr += size;
	  a.ptr = (void *) left_ptr;
	}
	while (cmp.func(cmp.data, mid, right_ptr) < 0) {
	  right_ptr -= size;
  ...
  /* Once the BASE_PTR  array is partially sorted by  quicksort the rest
     is completely sorted using  insertion sort, since this is efficient
     for  partitions  below  MAX_THRESH  size. BASE_PTR  points  to  the
     beginning of the array to sort, and END_PTR points at the very last
     element in the array (*not* one beyond it!). */
...
#define min(x, y) ((x) < (y) ? (x) : (y))

  {
    ...
    for (run_ptr = tmp_ptr + size; run_ptr <= thresh; run_ptr += size) {
      if (cmp.func(cmp.data, run_ptr, tmp_ptr) < 0)
	tmp_ptr = run_ptr;
    }
    if (tmp_ptr != base_ptr)
    ...
    while ((run_ptr += size) <= end_ptr) {
      ...
      while (cmp.func(cmp.data, run_ptr, tmp_ptr) < 0) {
	tmp_ptr -= size;
	b.ptr = (void *) tmp_ptr;
      }
      tmp_ptr += size;
